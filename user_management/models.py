...
class ERPUser(AbstractUser):
    ...
    # Override AbstractUser email to be unique
    email = models.EmailField(unique=True, validators=[EmailValidator()])
    
    # Map AbstractUser password field to the existing password_hash database column
    password = models.CharField(max_length=255, db_column='password_hash', default='')
    ...
    # Property wrapper to support direct usage of password_hash in views/serializers
    @property
...
class UserSession(models.Model):
    """
    Model to track user active sessions/devices
    """
    user = models.ForeignKey(
        ERPUser,
        on_delete=models.CASCADE,
    ...
    session_key = models.CharField(max_length=255, unique=True, help_text="Unique refresh token identifier (jti)")
...
# ============================================================================
# Signals for Session Revocation
# ============================================================================

@receiver(pre_save, sender=ERPUser)
def track_user_original_fields(sender, instance, **kwargs):
    """
    Pre-save signal to store original role and status before save
    """
    if instance.pk:
        try:
            orig = ERPUser.objects.get(pk=instance.pk)
            instance._original_role_id = orig.role_id
            instance._original_status = orig.status
            instance._original_is_active = orig.is_active
        except ERPUser.DoesNotExist:
            instance._original_role_id = None
...
@receiver(post_save, sender=ERPUser)
def revoke_sessions_on_admin_changes(sender, instance, created, **kwargs):
    """
    Post-save signal to instantly revoke all active sessions if user's role or status changes
    """
    if not created:
        role_changed = getattr(instance, '_original_role_id', None) != instance.role_id
        status_changed = getattr(instance, '_original_status', None) != instance.status
        is_active_changed = getattr(instance, '_original_is_active', None) != instance.is_active
        
        if role_changed or status_changed or is_active_changed:
            # Revoke all active sessions
            revoked_count = UserSession.objects.filter(user=instance, is_active=True).update(is_active=False)
            if revoked_count > 0:
                ActivityLog.objects.create(
                    user=instance,
                    action='OTHER',
                    module='Security',
                    description=f"Instantly revoked {revoked_count} active sessions due to status/role/activation modification.",
                    status='SUCCESS'
                )
